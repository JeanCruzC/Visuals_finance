...
st.set_page_config(page_title="Finanzas Personales — Dashboard", layout="wide")
...
def load_excel(file) -> dict:
    ...
    data["Inversiones"] = ensure_datetime(data["Inversiones"], "Fecha_Inversión")
...
def compute_ratios_and_balance(data: dict, dfm: pd.DataFrame, a: Assumptions):
    ...
    limite_total = float(tarjetas["Límite_Crédito"].sum()) if "Límite_Crédito" in tarjetas.columns else 0.0
    util_credito = safe_div(pasivos_tarjetas, limite_total)

    # Passive income & FI Index
    ing = data["Ingresos"].copy()
    passive = ing.loc[ing["Categoría"].astype(str).str.lower().isin(["inversiones","rentas"])].copy()
...
def build_debt_universe(data: dict):
    ...
    for _, r in deudas.iterrows():
        debts.append({
            "id": str(r["ID_Deuda"]),
            "name": f"{r.get('Tipo','Deuda')} — {r.get('Acreedor','')}".strip(),
            "balance": float(r.get("Saldo_Actual", 0.0)),
            "rate_monthly": float(r.get("Tasa_Interés_Anual", 0.0)) / 12.0,
    ...
    for _, r in tarjetas.iterrows():
        debts.append({
            "id": str(r["ID_Tarjeta"]),
            "name": f"Tarjeta {r.get('Banco_Emisor','')} •{r.get('Últimos_4_Dígitos','')}".strip(),
            "balance": float(r.get("Saldo_Actual", 0.0)),
            "rate_monthly": float(r.get("Tasa_Interés_Mensual", 0.0)),
            "min_payment": float(r.get("Pago_Mínimo", 0.0)),
            "kind": "card"
        })
...
def compute_health_score(dfm, ratios, data: dict):
    ...
    ing = data["Ingresos"].copy()
    shares = ing.groupby("Categoría")["Monto_Neto"].sum().values
    ...
    if len(dfm) >= 3:
        ...
        trend = clamp((x / max(1, dfm["Ingresos_Netos"].mean())) * 5 + 0.5, 0, 1)  # heurístico
    # Planning: budget adherence
    plan = float(dfm["Budget_Adherence"].mean()) if "Budget_Adherence" in dfm else 0.5
    # Education proxy: % spend on educación up to 10% => full score (heurístico)
    gv = data["Gastos_Variables"].copy()
    edu = gv.loc[gv["Categoría"] == "Educación", "Monto"].sum()
...
with st.sidebar:
    ...
    with st.expander("Configuración Avanzada (Opcional)"):
        ...
        expected_return = st.slider("Retorno esperado anual", 0.0, 0.30, 0.08, 0.005, help="Cuánto esperas ganar anualmente con tus inversiones (promedio).")
        inflation = st.slider("Inflación anual", 0.0, 0.20, 0.03, 0.005, help="Cuánto suben los precios cada año (promedio).")
        swr = st.slider("Safe Withdraw Rate (Retiro Seguro)", 0.02, 0.06, 0.04, 0.005, help="Porcentaje de tu portafolio que puedes gastar anualmente en tu jubilación sin que se acabe el dinero.")
        
        st.subheader("Activos y Runway")
        bienes = st.number_input("Valor de Bienes (Casa, Auto, etc.)", min_value=0.0, value=0.0, step=500.0, help="Valor estimado de tus activos físicos que podrías vender si fuera necesario.")
        haircut = st.slider("Castigo por liquidación rápida", 0.0, 1.0, 0.70, 0.05, help="Si tuvieras que vender tus inversiones hoy mismo por emergencia, ¿qué porcentaje del valor recuperarías? (0.70 = 70%)")
        ...
        st.subheader("Deuda — Pago Extra")
        extra_debt = st.number_input("Pago extra mensual a deudas", min_value=0.0, value=0.0, step=50.0, help="Dinero adicional que puedes destinar cada mes para pagar tus deudas más rápido.")

        st.subheader("Análisis de Costo de Oportunidad")
        oc_years = st.slider("Años a proyectar", 1, 30, 10, 1, help="Cuántos años en el futuro quieres ver el impacto de tus gastos.")
...
with tab1:
    ...
    fig_g1 = go.Figure(go.Indicator(
        mode="gauge+number",
        value=util*100,
        title={"text":"Utilización de crédito (%)"},
    ...
    # Net worth line
    fig_nw = px.line(nw_df, x="Mes", y="Patrimonio_Neto", title="Evolución de Patrimonio Neto (aprox)")
    fig_nw.update_layout(height=360, margin=dict(l=10,r=10,t=40,b=10))
    st.plotly_chart(fig_nw, use_container_width=True)

    st.caption("Asesor: Patrimonio mensual es aproximado porque tarjetas no tienen histórico y las inversiones usan valor actual como proxy por mes.")
...
with tab2:
    ...
    heat = mov2.groupby(["Dia","Categoría"])["Gasto_Pos"].sum().reset_index()
    if not heat.empty:
        pivot = heat.pivot_table(index="Dia", columns="Categoría", values="Gasto_Pos", aggfunc="sum").fillna(0.0)
        fig_heat = px.imshow(pivot.T, aspect="auto", title="Mapa de calor — Gastos por día y categoría")
...
with tab3:
    ...
    with c_pie1:
        fig_pie = px.pie(pie_assets, names="Tipo", values="Monto", title="Distribución de Activos", hole=0.3)
    ...
    st.write(
        ...
        f"- Ingreso mínimo para cubrir fijos (prom): **{money(fixed_avg, ass.currency_symbol)}**\n"
        f"- Días del mes trabajando “solo para obligaciones” (aprox): **{days_work:.1f} días**"
    )
...
with tab4:
    ...
    st.subheader("Análisis de deudas (incluye avalancha vs bola de nieve vs híbrido)")
    ...
    cB2.metric("Bola de nieve — meses a pagar", f"{months_sn}")
    cC2.metric("Híbrido — meses a pagar", f"{months_hy}")
    ...
    fig_pay = px.line(merged.melt(id_vars="month", var_name="Estrategia", value_name="Saldo"),
                      x="month", y="Saldo", color="Estrategia", title="Timeline de deuda (saldo total) — Gantt/curva de pago")
...
with tab6:
    ...
    proj12 = avg_flow * 12
    c1, c2 = st.columns(2)
    c1.metric("Ahorro proyectado 6 meses (prom últimos 3)", money(proj6, ass.currency_symbol))
    c2.metric("Ahorro proyectado 12 meses (prom últimos 3)", money(proj12, ass.currency_symbol))
    ...
    st.dataframe(metas[["Nombre_Meta","Categoría","Monto_Objetivo","Monto_Acumulado","Restante","Aporte_Mensual_Planeado","Meses_estimados","Prioridad","Estado"]],
    ...
    # Forecast net worth scenarios (1,5,10,20 years) — show 5-year curve
    st.markdown("### Forecast de patrimonio neto (3 escenarios) — 5 años")
    ...
    st.markdown("### Jubilación — referencia rápida (muy práctico)")
    exp_month = st.number_input("Gasto mensual objetivo en jubilación (referencia)", min_value=0.0, value=float(ratios["gasto_mensual_prom"]), step=100.0)
    ...
    st.markdown("### 12) Análisis de sensibilidad (what-if)")
    s1, s2, s3 = st.columns(3)
    job_loss_months = s1.slider("Pérdida de empleo (meses)", 0, 12, 6, 1)
    emergency_cost = s2.number_input("Emergencia médica (monto)", min_value=0.0, value=10000.0, step=500.0)
    rate_shock = s3.slider("Shock tasa interés mensual (tarjetas) +", 0.0, 0.05, 0.01, 0.005)
    ...
    st.write(
        ...
        f"- Shock de tasa tarjetas +{rate_shock*100:.1f}% mensual ≈ costo extra mensual: **{money(shock_cost, ass.currency_symbol)}**"
    )

    # Goal acceleration
    st.markdown("### 13) Aceleración de objetivos (10% + ahorro / - gastos)")
    delta = 0.10
    ...
    if len(metas) and chosen in metas["Nombre_Meta"].values:
        ...
        st.write(
            ...
            f"- Si aumentas ahorro 10%: **{m1} meses** (Δ {(m0-m1)} meses)\n"
            f"- Si reduces gastos 10%: **{m2} meses** (Δ {(m0-m2)} meses)\n"
            f"- Si haces ambos: **{m3} meses** (Δ {(m0-m3)} meses)"
        )

    # Tax efficiency quick view
    st.markdown("### 14) Eficiencia fiscal (básico con tu data)")
    ing = data["Ingresos"].copy()
    ...
    st.caption("Estimación de tramo (referencial):")
    if eff_tax < 0.08:
        st.info("Tramo Bajo: Parece que pagas pocos impuestos. ¡Aprovecha para ahorrar más!")
    elif eff_tax < 0.20:
        st.warning("Tramo Medio: Estás en el promedio. Revisa si puedes deducir gastos de educación o salud.")
    else:
